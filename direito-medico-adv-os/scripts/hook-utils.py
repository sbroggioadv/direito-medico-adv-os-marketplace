...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando direito-medico/ acima
- hook_state_load/save    → gerencia direito-medico/.hook-state.json (debounce anti-flap)
...
STATE_DIR = "direito-medico"
HOOK_STATE_FILENAME = ".hook-state.json"


def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/direito-medico/cowork-state.json."""
    start = start.resolve()
